...
function getTagAndModifyTime() {
  ...
  $.getJSON("/api/v1/article/view?id=" + id, function (result) {
    $article.append(
      "<blockquote class='float-end'><h5 class='post-meta'>Last Modified:</h5>" +
        "<p class='float-end post-meta'>" +
        moment.utc(result.data.modify_time).local().format() +
        "</p></blockquote>",
    );
    var tags = { data: [] };
    ...
    $article.children().first().after(html);
  });
}
